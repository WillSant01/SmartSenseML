...
class GestureGRUClassifier:
    ...
    def load_and_preprocess_data(self, rest_folder, gesture_folders):
        """Load data from all folders, using only 5 random rest files."""
        if self.verbose:
        ...
        # Get all rest files and randomly select 5
        all_rest_files = sorted(glob.glob(os.path.join(rest_folder, "*.csv")))
        if len(all_rest_files) > 5:
            rest_files = random.sample(all_rest_files, 5)
        else:
            rest_files = all_rest_files
            
        if self.verbose:
            print(f"Using {len(rest_files)} rest files:")
            for f in rest_files:
        ...
        for file in rest_files:
            try:
                ...
                df = pd.read_csv(file)
                data = df[['AccX', 'AccY', 'AccZ', 'GyroX', 'GyroY', 'GyroZ', 'label']].values
                ...
                for i in range(0, len(data) - self.window_size + 1, stride):
                    ...
                    if len(sequence) == self.window_size:
                        ...
                        if np.all(sequence[:, -1] == 0):  # all labels are 0
                            sensor_sequence = sequence[:, :6]  # All sensor data
                            all_sequences.append(sensor_sequence)
                            all_labels.append(0)
    ...
    def build_model(self):
        ...
        inputs = Input(shape=(self.window_size, 6))  # 6 for AccX, AccY, AccZ, GyroX, GyroY, GyroZ
